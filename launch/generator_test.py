...
tf.app.flags.DEFINE_bool('resume_training', False, 'resume training') # Starts a new training process, overwritting the prev checkpoint
tf.app.flags.DEFINE_integer('batch_size', 2, 'batch size')
tf.app.flags.DEFINE_bool('fine_tune', True, 'Fine-tune head layers (heatmap generation)') # only train last layers
...
def test(args):
    ...
    * unused_argv: unused arguments (after FLAGS is parsed)
    ...
    _, logits, labels, ids = trainer.sess_eval.run(trainer.eval_op)
